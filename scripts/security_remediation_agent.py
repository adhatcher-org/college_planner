#!/usr/bin/env python3
"""Dependabot security remediation agent for the College Planner repo."""
...
def _load_uv_lock_versions(lock_path: Path) -> dict[str, str]:
...
def _load_npm_lock_versions(lock_path: Path) -> dict[str, str]:
    if not lock_path.exists():
        return {}
    content = json.loads(lock_path.read_text(encoding="utf-8"))
    result: dict[str, str] = {}
    for package_path, package in content.get("packages", {}).items():
        if not package_path.startswith("node_modules/"):
            continue
        package_name = package_path.removeprefix("node_modules/")
        package_version = package.get("version")
        if package_name and isinstance(package_version, str):
            result[_normalize_package_name(package_name)] = package_version
    return result
...
def _passes_filters(fields: dict[str, Any], min_severity: str) -> bool:
    if fields.get("state") != "open":
        return False
    if fields.get("ecosystem") not in SUPPORTED_ECOSYSTEMS:
        return False
    severity_rank = SEVERITY_ORDER.get(fields.get("severity", ""), 0)
...
def _api_get_json_with_headers(
    ...
    try:
        ...
        if exc.code == 403 and "Resource not accessible by integration" in detail:
            hint = " Hint: set DEPENDABOT_ALERTS_TOKEN with Dependabot alerts read permission."
...
def _build_python_constraint(first_patched_version: str) -> str | None:
...
def _build_npm_target(package_name: str, first_patched_version: str) -> str:
    semver = _parse_semver(first_patched_version)
    if semver:
        major, minor, patch = semver
        return f"{package_name}@^{major}.{minor}.{patch}"
...
def _format_pr_body(
    ...
    validation = ["- `make test`", "- `make security`"]
    lines = [
        "## Automated Security Remediation",
        "",
        f"- Alert: `{fields['alert_id']}`",
        f"- Package: `{fields['package']}`",
        f"- Ecosystem: `{fields['ecosystem']}`",
        f"- Manifest: `{fields['manifest_path'] or 'unknown'}`",
...
def _project_for_alert(fields: dict[str, Any]) -> tuple[Path, Path, Path]:
    ecosystem = fields["ecosystem"]
    manifest_path = fields.get("manifest_path") or ""
    if ecosystem == "npm" or manifest_path.startswith("frontend/"):
        return Path("frontend"), Path("frontend/package.json"), Path("frontend/package-lock.json")
    return Path("backend"), Path("backend/pyproject.toml"), Path("backend/uv.lock")


def _version_map_for_alert(fields: dict[str, Any], lock_path: Path) -> dict[str, str]:
    if fields["ecosystem"] == "npm":
        return _load_npm_lock_versions(lock_path)
    return _load_uv_lock_versions(lock_path)


def _run_update(fields: dict[str, Any], project_dir: Path) -> list[subprocess.CompletedProcess[str]]:
    package_name = fields["package"]
    if fields["ecosystem"] == "npm":
        runs = [_run(["npm", "update", package_name], cwd=project_dir)]
        if runs[0].returncode != 0 or fields["first_patched_version"]:
            runs.append(_run(["npm", "install", _build_npm_target(package_name, fields["first_patched_version"])], cwd=project_dir))
        return runs

    runs = [_run(["uv", "lock", "--upgrade-package", package_name], cwd=project_dir)]
    if runs[0].returncode != 0 and fields["first_patched_version"]:
        constraint = _build_python_constraint(fields["first_patched_version"])
        if constraint:
            runs.append(_run(["uv", "add", f"{package_name}{constraint}"], cwd=project_dir))
    return runs
...
def _remediate_alert(args: argparse.Namespace) -> int:
    ...
    if not alert:
        _write_json(Path(args.output), {"status": "skipped", "reason": f"Alert {args.alert_id} is not open or not found."})
        return 0
    ...
    if not _passes_filters(fields, args.severity_threshold):
        _write_json(
            Path(args.output),
            {"status": "skipped", "reason": f"Alert {args.alert_id} does not match filters.", "alert": fields},
        )
        return 0

    package_name = fields["package"]
    if not package_name:
        _write_json(Path(args.output), {"status": "skipped", "reason": "Alert has no package name.", "alert": fields})
        return 0
    ...
    project_dir, manifest_path, lock_path = _project_for_alert(fields)
    ...
    base_result: dict[str, Any] = {
        ...
        "commands": [],
        "reason": "",
        "changed_files": [str(manifest_path), str(lock_path)],
    }
    ...
    before_versions = _version_map_for_alert(fields, lock_path)
    package_key = _normalize_package_name(package_name)
    old_version = before_versions.get(package_key)

    update_runs = _run_update(fields, project_dir)
    for completed in update_runs:
        command_text = " ".join(shlex.quote(part) for part in completed.args)
        base_result["commands"].append(f"(cd {project_dir} && {command_text})")

    changed_files = _run(["git", "status", "--porcelain", "--", str(manifest_path), str(lock_path)])
    ...
    if not changed_files.stdout.strip():
        stderr_text = "\n".join([attempt.stderr.strip() for attempt in update_runs if attempt.stderr.strip()])
        base_result["status"] = "skipped"
        base_result["reason"] = "Remediation produced no dependency changes." + (f" Last error: {stderr_text}" if stderr_text else "")
        _write_json(Path(args.output), base_result)
        return 0

    quality_commands = [["make", "test"], ["make", "security"]]
    ...
    after_versions = _version_map_for_alert(fields, lock_path)
